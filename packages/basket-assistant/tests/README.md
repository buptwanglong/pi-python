...
This directory contains the test suite for basket-assistant. The tests ensure the agent's core functionality, tools, and integrations work correctly.
    ...
    steps:
      ...
      - name: Install dependencies
        run: |
          ...
          poetry install
      - name: Run tests
        run: poetry run pytest -v --cov=basket_assistant
```
...
**Workaround:** Update tests to use `from basket_ai import api` and patch `api.get_model`.
