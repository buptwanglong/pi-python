...
```bash
poetry run python -m basket_assistant "List all Python files in this directory"
```
...
# Run with coverage
poetry run pytest --cov=basket_assistant --cov-report=term
```
...
- `~/.basket-assistant/settings.json` - Your settings
- `basket_assistant/main.py` - Main agent logic
- `basket_assistant/modes/tui.py` - TUI display
