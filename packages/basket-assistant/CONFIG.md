...
- **CLI 切换**：运行 `basket --agent <名称>` 时，会设置环境变量 `BASKET_AGENT`，当前进程的主 Agent 将使用该名称解析 `agents[名称].model`（若存在），否则仍回退到 `default_agent` 或顶层 `model`。
...
在工作区目录下可放置以下 Markdown 文件（UTF-8），缺失的文件会被忽略，仅存在且有内容的文件会参与组装。约定仿照 OpenClaw 的 Agent workspace 文件。
...
| **TOOLS.md** | （可选）本地工具与环境约定：路径、习惯、适配器行为等；仅作提示，不控制工具是否可用。 |
| **MEMORY.md** | （可选）长期记忆：持久事实、压缩后的历史摘要；建议只读注入。 |

组装顺序为：Identity → Soul → Operating instructions → User context，若存在则追加 TOOLS、MEMORY，最后固定追加工具使用说明（read/write/edit/bash/grep/skill 等），保证模型始终知道可用工具。
