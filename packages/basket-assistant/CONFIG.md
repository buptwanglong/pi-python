...
首次使用建议运行 **`basket init`** 进行引导式配置，按提示选择 Provider、填写 API Key、模型、工作区与 Web 搜索等，即可在 `~/.basket/settings.json` 生成完整配置（也可通过 `basket init --path <文件路径>` 指定输出文件，`basket init --force` 覆盖已有文件而不二次确认）。实现上由统一配置模块 `core.configuration.ConfigurationManager` 与 `ConfigInitializer` 完成。完成后运行 `basket` 即可启动；后续可按需直接编辑 `settings.json` 或使用 `basket agent list/add/remove` 管理子智能体。
...
- **固定搜索路径**（不可通过 `skills_dirs` 覆盖；该字段已忽略）：`~/.basket/skills`、`./.basket/skills`，以及已安装插件（`~/.basket/plugins/<name>/skills/`）下的技能目录。同技能名时**后出现的目录覆盖先出现的**。若需使用旧版 OpenCode/Claude 等路径下的技能，请将目录**符号链接**到 `~/.basket/skills` 或项目的 `.basket/skills`。
...
- **配置**：`skills_include`（要纳入索引的技能名列表，空则全部）。`skills_dirs` 保留在 schema 中但**不再生效**，请勿依赖。

## 声明式 SlashCommand（`*.md`）

与内置斜杠命令（`/help`、`/model` 等）不同：用户可在固定目录放置 **Markdown 文件**，文件名（不含扩展名）即命令名，例如 `review.md` → `/review`。

- **固定目录**（不可配置）：`~/.basket/commands`、`./.basket/commands`，以及各插件的 `commands/`（`~/.basket/plugins/<name>/commands/`）。同名命令时**后扫描的目录覆盖先前的**。
- **文件格式**：`---` YAML 片段 + 正文。必填 frontmatter：`description:`。可选：`skill:`（合法技能 id，本回合与 `/skill` 相同，会把该技能全文注入 system prompt）、`disable-model-invocation:`（布尔，当前仅作元数据）。正文支持占位符 `{{args}}`（对应 `/命令名` 后面余下的整段文本）。
- **路由优先级**（交互模式）：pending 问答 → 内置 Command → `/skill` → 声明式命令 → 未知 `/` 报错 → 普通输入。

## 常驻助理 (basket gateway)

- 端口：环境变量 `BASKET_SERVE_PORT`（默认 7682）；状态文件 `~/.basket/serve.pid`、`~/.basket/serve.port`。
- 使用方式：运行 `basket tui` 即可（若 gateway 未运行会先后台启动再连接）；或单独 `basket gateway start` 启动、`basket gateway stop` 停止、`basket gateway status` 查看状态。`basket tui-native`（或 `basket tn`）与 `basket tui` 共用同一 gateway，输出为逐行可选中复制的终端原生界面。
