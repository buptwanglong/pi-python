# Example Extensions for Basket

This directory contains example extensions that demonstrate how to extend Basket with custom tools, commands, and event handlers.
...
**Usage:**
```python
# Copy to ~/.basket/extensions/ or ./extensions/
...
- Storage: `.basket/project_context.json`
...
def setup(basket):
    ...
    # Register a command
    @basket.register_command("/your-cmd")
...
- `@basket.register_tool(name, description, parameters)` - Register a tool
- `@basket.register_command(command)` - Register a slash command
- `@basket.on(event_name)` - Register an event handler
...
**API methods:**
- `basket.get_context()` - Get agent context
- `basket.get_settings()` - Get agent settings
- `basket.get_session_manager()` - Get session manager
- `basket.execute_command(command, args)` - Execute a command
- `basket.get_commands()` - List registered commands

### Installation

1. **User-level extensions** (available in all projects):
   ```bash
   mkdir -p ~/.basket/extensions
   cp your_extension.py ~/.basket/extensions/
   ```
