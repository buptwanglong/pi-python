...
from pi_coding_agent.extensions import ExtensionAPI, ExtensionLoader
...
def test_extension_loader_load_extensions_from_dir(tmp_path, capsys):
    ...
    ext1.write_text("def setup(pi): pass")

    ext2 = tmp_path / "ext2.py"
    ext2.write_text("def setup(pi): pass")
...
def test_extension_loader_load_default_extensions(tmp_path, monkeypatch, capsys):
    ...
    # Mock home and cwd
    user_ext_dir = tmp_path / ".pi" / "extensions"
    ...
    # Create test extensions
    (user_ext_dir / "user_ext.py").write_text("def setup(pi): pass")
    (local_ext_dir / "local_ext.py").write_text("def setup(pi): pass")
